# Task 004: Architecture Documentation Update
...
The ARCHITECTURE.md document lists "Alert delivery workers and integrations" as a non-goal, but these are now fully implemented. The document also doesn't mention the customer plane, SNMP support, or RLS enforcement.

**Read first**:
- `docs/ARCHITECTURE.md` (current state)
- `docs/CUSTOMER_PLANE_ARCHITECTURE.md` (customer plane design)
- `docs/INTEGRATIONS_AND_DELIVERY.md` (delivery design)
...
### 4.1 Replace ARCHITECTURE.md with updated content

Replace the entire `docs/ARCHITECTURE.md` with:
...
- [ ] Architecture diagram shows full flow including delivery
- [ ] All services documented (including dispatcher, delivery_worker)
- [ ] Authentication model documented
- [ ] RLS security model documented
- [ ] Alert delivery flow documented
- [ ] "Alert delivery workers" removed from non-goals
- [ ] SNMP mentioned as delivery option
...
```
Update ARCHITECTURE.md with complete system design

- Add architecture diagram with delivery flow
- Document dispatcher and delivery_worker services
- Add authentication model section
- Document RLS security architecture
- Add alert delivery flow
- Remove delivery from non-goals (now implemented)
