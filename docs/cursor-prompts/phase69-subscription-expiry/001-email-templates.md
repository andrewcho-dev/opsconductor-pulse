# Prompt 001 — Email Templates for Expiry Notifications

Read `services/subscription_worker/worker.py` fully.
Read `services/delivery_worker/email_sender.py` to understand the email sending pattern.

## Create `services/subscription_worker/email_templates.py`
...
<head><style>
  ...
  .cta {{ background: #1a56db; color: white; padding: 10px 20px;
...
- [ ] `services/subscription_worker/email_templates.py` created
- [ ] Templates for pre-expiry and grace period notifications
- [ ] Subject/HTML/text templates defined
