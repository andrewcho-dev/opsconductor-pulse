# Task 1: Replace DeviceListPage with Full-Width DataTable
...
1. Custom hand-rolled card buttons in a scrollable 380px sidebar — not a DataTable
2. No sortable columns, no column headers, no "Showing X of Y"
3. Master-detail split crams a 6-tab detail pane into the remaining width
4. The detail pane (`DeviceDetailPane`) has completely different tabs from the actual `DeviceDetailPage` — two divergent implementations
5. Pagination is just prev/next icon buttons with no page info
6. On mobile, clicking a device navigates to the full page anyway — proving the split is unnecessary

## Goal

Replace the entire page with a full-width DataTable matching TemplateListPage's pattern:
- Sortable columns: Status, Device ID, Template, Site, Last Seen, Alerts
- `onRowClick` → `navigate(\`/devices/${device.device_id}\`)`
- Server-side pagination with `totalCount`, page size selector, "Showing X–Y of Z"
- Filter bar above the table: search input + status dropdown + site dropdown
- Health strip above the filters (existing)
...
export default function DeviceListPage({ embedded }: { embedded?: boolean }) {
  ...
  const columns = useMemo<ColumnDef<Device>[]>(
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-sm">
            {row.original.template?.name ?? row.original.template_name ?? row.original.model ?? "—"}
          </span>
        ),
      },
      {
        ...
        header: "Site",
        cell: ({ row }) => {
          const site = (sitesData?.sites ?? []).find(
            (s) => s.site_id === row.original.site_id
          );
          return (
            <span className="text-sm">{site?.name ?? row.original.site_id ?? "—"}</span>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-sm text-muted-foreground">
            {formatTimeAgo(row.original.last_seen_at)}
          </span>
        ),
      },
      {
    ...
    [alertCountByDevice, sitesData?.sites]
  );
...
| Aspect | Old | New |
|--------|-----|-----|
| Layout | 380px sidebar + detail pane | Full-width DataTable |
| Device click | Select in sidebar (desktop) or navigate (mobile) | Always navigate to `/devices/:deviceId` |
| Columns | None — cards with scattered info | 7 sortable columns |
| Pagination | Prev/Next icon buttons | "Showing 1–25 of 142" with page size selector |
| Detail view | Cramped `DeviceDetailPane` (different from detail page) | Uses the existing rich `DeviceDetailPage` |
| Dependencies | DeviceDetailPane + all its sub-imports | None — just DataTable |
...
- Device list shows full-width DataTable with 7 columns
- Columns are sortable (click header)
- Row click navigates to `/devices/:deviceId`
- Health strip shows Online/Stale/Offline counts
- Search debounces and filters
- Status and Site dropdowns filter
- Pagination shows "Showing X–Y of Z" with page size selector (10/25/50/100)
- Empty state shows when no devices match filters
