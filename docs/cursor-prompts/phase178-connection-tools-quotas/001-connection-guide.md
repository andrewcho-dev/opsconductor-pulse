# Task 1: Connection Guide Page

## Objective

Create a Connection Guide page that shows language-specific code snippets for connecting devices to the platform. Includes Python, Node.js, curl (HTTP ingestion), and Arduino/ESP32 examples with copy-to-clipboard functionality.
...
The page has:
1. A brief intro explaining the connection flow (provision device → get credentials → connect)
2. A "Connection Parameters" card showing the platform's broker info
3. Language selector tabs (Python, Node.js, curl, Arduino) with syntax-highlighted code blocks
4. Copy-to-clipboard button on each code block
    ...
    <div className="space-y-4">
      {!embedded && (
        <PageHeader
          ...
          description="Connect your devices to OpsConductor Pulse"
        />
      )}
        ...
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          <div className="flex items-start gap-3">
            ...
            <span>
              ...
              <Link to="/devices/wizard" className="text-primary hover:underline">Devices page</Link>{" "}
          ...
          <div className="flex items-start gap-3">
            ...
            <span>
              <strong className="text-foreground">Connect via MQTT</strong> using the credentials and code snippets below.
            </span>
          </div>
          <div className="flex items-start gap-3">
            ...
            <span>
              ...
              <code className="text-xs bg-muted px-1 py-0.5 rounded">tenant/TENANT_ID/device/DEVICE_ID/telemetry</code>{" "}
              as JSON.
            </span>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Replace <code>your-host</code> with your platform's hostname. Get device credentials from the Devices page.
          </p>
        </CardContent>
      </Card>
...
**Notes:**
- The `embedded` prop is included so this page can be rendered inside the Tools hub
- The code snippets use placeholder values (`your-broker-host`, `your-client-id`, etc.) — users replace them with their actual credentials
- Verify the actual topic format by checking `services/ui_iot/routes/internal.py` (MQTT ACL) and `services/ui_iot/routes/ingest.py` (HTTP ingestion)
- The `CopyButton` component uses the Clipboard API
- Code blocks use `<pre>` with monospace font — no syntax highlighting library is needed (keeps it lightweight)

## Verification

- `npx tsc --noEmit` passes
- File created at correct path
- All 4 language tabs render with code blocks
- Copy button copies text to clipboard
- Links to Devices page work
