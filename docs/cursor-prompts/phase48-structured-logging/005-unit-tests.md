# Prompt 005 — Unit Tests for JSON Log Format

## Your Task

Add `tests/unit/test_structured_logging.py`. Follow existing patterns: `pytestmark = [pytest.mark.unit, pytest.mark.asyncio]`.
...
def test_log_exception_helper():
    ...
    try:
        ...
        log_exception(logger, "operation failed", e, context={"tenant_id": "t1"})
    line = get_log_line(buf)
    assert line["level"] == "ERROR"
...
- [ ] `tests/unit/test_structured_logging.py` exists with 7 tests
- [ ] All tests pass: valid JSON, correct fields, correct levels, single line per record
