...
Radar (spider) charts show multiple metrics on radial axes — perfect for "device health at a glance" where you want to see temperature, humidity, uptime, battery, signal strength all in one visualization. Used by ThingsBoard, Grafana, and most IoT dashboards.

## Step 1: Create RadarRenderer.tsx

**File:** `frontend/src/features/dashboard/widgets/renderers/RadarRenderer.tsx` (NEW)
...
// Reasonable max values for common IoT metrics (used as radar axis max)
...
export default function RadarRenderer({ config }: WidgetRendererProps) {
  const metrics = Array.isArray(config.radar_metrics) && config.radar_metrics.length >= 3
  ...
  const option = useMemo<EChartsOption>(() => {
    // Build indicator (radar axes) and values
    ...
    const values = queries.map((q) => {
      ...
      if (!response?.summary?.avg) return 0;
      return response.summary.avg;
    });
    ...
    return {
      ...
      legend: showLegend
        ? { bottom: 0, textStyle: { color: isDark ? "#a1a1aa" : "#52525b", fontSize: 11 } }
        : { show: false },
      radar: {
        indicator,
        shape: "polygon",
...
|-------|------|---------|-------------|
| `radar_metrics` | `string[]` | `["temperature", "humidity", "pressure"]` | 3-6 metrics to display on radar axes |
| `time_range` | `string` | `"24h"` | Time window for averaging |
| `show_legend` | `boolean` | `true` | Show legend |

## Key design decisions

1. Uses `useQueries` for parallel metric fetches — one analytics query per radar axis
2. Each metric has a default max value for reasonable axis scaling
3. Polygon shape (not circle) for clearer IoT metric comparison
4. Filled area with low opacity for visual weight
5. Dark/light theme-aware grid and label colors
