...
Scatter plots show correlation between two metrics — essential for IoT analytics (e.g., temperature vs humidity, power vs vibration). Each dot represents a device's metric values, helping operators spot patterns and outliers.

## Step 1: Create ScatterRenderer.tsx

**File:** `frontend/src/features/dashboard/widgets/renderers/ScatterRenderer.tsx` (NEW)
...
export default function ScatterRenderer({ config }: WidgetRendererProps) {
  ...
  // Fetch X-axis metric (grouped by device to get per-device averages)
  ...
  const option = useMemo<EChartsOption>(() => {
    ...
    // Build a map of device label -> average value for each metric
    ...
    // Merge: only include devices that have both metrics
...
1. Two parallel analytics API calls with `group_by: "device"`
2. Each returns per-device time series
3. We average each device's values to get one (x, y) point per device
4. Devices with only one metric are excluded
5. Tooltip shows device name + both metric values

## Config fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `x_metric` | `string` | `"temperature"` | Metric for X axis |
| `y_metric` | `string` | `"humidity"` | Metric for Y axis |
