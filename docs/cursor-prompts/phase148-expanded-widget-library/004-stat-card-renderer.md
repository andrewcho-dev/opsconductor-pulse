...
Every modern dashboard platform (Grafana, Datadog, AWS CloudWatch, Azure IoT Central) offers KPI tiles with inline sparkline trends — a small chart behind the value showing recent trajectory. Our current KPITileRenderer shows only a static number. The StatCardRenderer adds a trend sparkline and directional arrow, which is the single most requested dashboard feature in IoT platforms.

## Step 1: Create StatCardRenderer.tsx

**File:** `frontend/src/features/dashboard/widgets/renderers/StatCardRenderer.tsx` (NEW)
...
import { fetchFleetSummary, getFleetUptimeSummary, fetchTelemetryHistory } from "@/services/api/devices";
...
export default function StatCardRenderer({ config }: WidgetRendererProps) {
  ...
  // Fetch recent history for sparkline (use first device or fleet-level metric)
  const sparklineDeviceId = typeof config.sparkline_device === "string" ? config.sparkline_device : undefined;
  ...
  const trend = useMemo(() => {
    ...
    const avgRecent = recent.reduce((s, p) => s + (p.avg ?? 0), 0) / recent.length;
    const avgEarlier = earlier.reduce((s, p) => s + (p.avg ?? 0), 0) / earlier.length;
  ...
  const hasSparkline = (historyData?.points ?? []).length > 1;
        ...
        <div className="flex items-center justify-center gap-1 text-xs text-muted-foreground mt-0.5">
          ...
          {hasSparkline && (
            <span className="flex items-center gap-0.5 ml-1">
              ...
              {trend.pct !== 0 && (
                <span className={trend.direction === "up" ? "text-green-500" : trend.direction === "down" ? "text-red-500" : ""}>
...
1. **Sparkline background** — small area chart in bottom half, low opacity, shows 24h trend
2. **Trend arrow** — TrendingUp/TrendingDown/Minus icon based on recent vs earlier data
3. **Trend percentage** — calculated from first quarter vs last quarter of data points
4. **Threshold coloring** — same as KPITileRenderer
5. **Optional sparkline** — only shows if `sparkline_device` is set in config and returns data
