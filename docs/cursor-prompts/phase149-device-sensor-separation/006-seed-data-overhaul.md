# Task 006 — Seed Data Overhaul: Gateways, Sensors, Connections, Health

## Migration File

Create `db/migrations/104_seed_device_sensor_data.sql`

## Context

The existing seed data (migration 052) creates 12 `SENSOR-*` entries in `device_registry` — all treating sensors as devices. This migration replaces that with a proper hierarchy:

- **4 gateway devices** with distinct hardware identities and cellular connections
- **16 sensors** distributed across the gateways
- **Seed device_connections** for each gateway
- **Seed device_health_telemetry** with sample platform diagnostics

Existing SENSOR-001 through SENSOR-012 records will be deleted (user confirmed all current data is expendable).
...
| Gateway | Location | Sensors | Connection | Status |
|---------|----------|---------|------------|--------|
| GW-001 | Bldg A Server Room/HVAC | 5 (temp, humidity, pressure, HVAC supply/return) | Hologram LTE-M, 500MB | ONLINE |
| GW-002 | Bldg B Lab/Machinery | 5 (temp, humidity, vibration RMS/peak, coolant flow) | Hologram 4G, 500MB | ONLINE |
| GW-003 | Bldg C New Wing | 2 (temp, humidity) | 1NCE NB-IoT, 500MB | STALE (battery low, weak signal) |
| GW-004 | Bldg A Electrical | 4 (power kW, power factor, voltage L1, current) | Hologram 4G, 2GB | ONLINE |

This gives realistic variety:
- Different gateway models and chipsets
- Different carriers (Hologram, 1NCE)
- Different network types (LTE-M, 4G, NB-IoT)
- Different power sources (line, battery, PoE)
- Different sensor counts per gateway (2, 4, 5)
- One stale gateway with low battery and weak signal
- Multiple sensors of same type on different devices (temperature appears on all 4)
- Multiple sensors of same type on one device (GW-001 has 3 temperature sensors with different metric_names)

## Verification

```bash
psql -d iot -f db/migrations/104_seed_device_sensor_data.sql

# Verify gateways
psql -d iot -c "SELECT device_id, device_type, model, status FROM device_registry WHERE tenant_id = 'acme-industrial' ORDER BY device_id;"

# Verify sensors
psql -d iot -c "SELECT device_id, metric_name, sensor_type, label, unit, last_value FROM sensors WHERE tenant_id = 'acme-industrial' ORDER BY device_id, metric_name;"

# Verify connections
psql -d iot -c "SELECT device_id, carrier_name, connection_type, network_status, data_used_mb FROM device_connections WHERE tenant_id = 'acme-industrial' ORDER BY device_id;"

# Verify health telemetry
psql -d iot -c "SELECT device_id, rssi, signal_quality, battery_pct, cpu_temp_c FROM device_health_telemetry WHERE tenant_id = 'acme-industrial' ORDER BY device_id, time DESC;"

# Verify old SENSOR-* records are gone
psql -d iot -c "SELECT COUNT(*) FROM device_registry WHERE tenant_id = 'acme-industrial' AND device_id LIKE 'SENSOR-%';"
# Expected: 0
```
