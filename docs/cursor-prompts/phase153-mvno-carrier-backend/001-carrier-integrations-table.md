# Task 001 — Carrier Integrations Table

## Migration File

Create `db/migrations/106_carrier_integrations.sql`
...
- `api_key` and `api_secret` should be encrypted at the application level before storage. Use the same encryption pattern as any existing secrets in the codebase (check for `fernet` or similar).
- The `config` JSONB field holds carrier-specific settings that differ between providers.
- `carrier_device_id` on `device_connections` allows mapping our device_id to the carrier's internal ID (e.g., Hologram uses numeric device IDs, Jasper uses ICCIDs).

## Verification

```bash
psql -d iot -f db/migrations/106_carrier_integrations.sql
psql -d iot -c "\d carrier_integrations"
psql -d iot -c "\d device_connections" | grep carrier
```
