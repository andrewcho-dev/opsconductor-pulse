# 014: Operator Edit Subscription Dialog

## Task

Create a dialog for operators to manage tenant subscriptions with full audit logging.

## File to Create

`frontend/src/features/operator/EditSubscriptionDialog.tsx`

## Requirements

1. Edit device limit, term dates, status
2. Quick actions: Extend 30/90/365 days, Activate, Suspend
3. Require reason/notes for audit trail
4. Optional transaction reference field
5. Show confirmation before destructive actions
...
export function EditSubscriptionDialog({
  ...
  const handleSave = () => {
    ...
    // Require confirmation if reducing device limit below current count
    if (
      subscription &&
            ...
            <div className="space-y-2">
              ...
              {subscription && parseInt(deviceLimit, 10) < subscription.active_device_count && (
                <p className="text-xs text-orange-600">
                  Warning: Current usage ({subscription.active_device_count}) exceeds this limit.
                  Tenant will need to remove devices.
                </p>
              )}
            </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingAction?.type === "status_change" && (
                <>
                  You are about to set the subscription status to{" "}
                  <strong>{status}</strong>. This will:
              ...
              {pendingAction?.type === "limit_reduction" && (
                <>
                  ...
                  {subscription?.active_device_count}). The tenant will need to
                  remove devices to comply.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
...
Update `services/ui_iot/routes/operator.py` to accept `notes` and `transaction_ref` in the subscription update and include them in the audit log:
...
class SubscriptionUpsert(BaseModel):
    ...
    transaction_ref: Optional[str] = None  # ADD THIS
```

And in the audit log insert, add to the `details` JSONB:
```python
details = {
    "notes": data.notes,
...
1. Navigate to Operator → Tenants → click a tenant
2. Click "Manage" on the Subscription card
3. Try quick actions: +30 Days, Activate, Suspend
4. Verify notes are required
5. Verify confirmation appears for SUSPENDED status
6. Check subscription_audit table for logged entries with notes
