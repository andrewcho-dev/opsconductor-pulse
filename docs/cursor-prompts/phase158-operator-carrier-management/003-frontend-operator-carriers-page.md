# Task 3: Frontend — OperatorCarriersPage Component

## File to Create

`frontend/src/features/operator/OperatorCarriersPage.tsx`

## What to Do

Create a new operator page for managing carrier integrations across all tenants. Follow the `SubscriptionsPage.tsx` pattern exactly for structure, but add CRUD dialogs (create, edit, delete) since this page needs write operations.

### Reference File

Read `frontend/src/features/operator/SubscriptionsPage.tsx` as the structural template. This page extends that pattern with:
- A "carrier name" filter dropdown (in addition to the tenant ID filter)
- Create/Edit/Delete actions (SubscriptionsPage is read-only; this page is CRUD)
...
```tsx
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, ...rest }: { id: number } & Parameters<typeof updateOperatorCarrierIntegration>[1]) =>
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { invalidate(); setDeleteRow(null); },
  });
```
        ...
        <Dialog open={!!deleteRow} onOpenChange={(open) => { if (!open) setDeleteRow(null); }}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Delete "{deleteRow.display_name}" ({deleteRow.carrier_name}) for tenant {deleteRow.tenant_id}?
                This will unlink all associated devices.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
...
### Sub-components — CreateCarrierDialog

Define as a function component in the same file, below the default export:
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            <Label>Carrier *</Label>
            <Select value={form.carrier_name} onValueChange={(v) => setForm({ ...form, carrier_name: v })}>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={isPending || !form.tenant_id || !form.display_name}>
...
### Sub-components — EditCarrierDialog

Similar to Create, but pre-populated from `integration` prop, and only sends changed fields:
...
function EditCarrierDialog({
  ...
  const [form, setForm] = useState({
    ...
    api_key: "",  // blank = don't update; user types new value to change
  ...
  const handleSubmit = () => {
    ...
    if (form.api_key) updates.api_key = form.api_key; // only send if user typed a new key
    ...
    if (form.sync_interval_minutes !== integration.sync_interval_minutes) updates.sync_interval_minutes = form.sync_interval_minutes;
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <Input value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} placeholder={integration.api_key_masked ?? "Not set"} />
          </div>
          <div className="grid gap-2">
            <Label>Account ID</Label>
            <Input value={form.account_id} onChange={(e) => setForm({ ...form, account_id: e.target.value })} />
...
- Check what UI components are available in `frontend/src/components/ui/` — specifically `dialog.tsx`, `switch.tsx`, `label.tsx`. If any are missing, the project likely uses shadcn/ui so they should exist.
- The `PageHeader` component is imported from `@/components/shared` — same as `SubscriptionsPage`.
- `useQueryClient` is needed for mutation invalidation — import from `@tanstack/react-query`.
- Match the exact import paths used in other operator pages.
- The `colSpan` values must match the number of table columns (10 in this case).
