# Task 003 — Entitlements Middleware Overhaul

## File

Rewrite `services/ui_iot/middleware/entitlements.py`

## Purpose

Replace the old entitlements module (which read from `subscription_plans` and `subscriptions`) with one that checks the two-tier model: `account_tiers` for tenant-level limits/features, `device_plans` for per-device limits/features.
...
async def get_account_usage(conn, tenant_id: str) -> dict:
    ...
    user_count = await conn.fetchval(
        "SELECT COUNT(DISTINCT user_id) FROM user_role_assignments WHERE tenant_id = $1", tenant_id
    )
...
| Old | New |
|---|---|
| `check_device_limit(conn, tenant_id)` — checked subscription device_limit | Removed — devices aren't counted against a tenant limit anymore |
| `check_alert_rule_limit(conn, tenant_id)` — read from subscription_plans.limits | `check_alert_rule_limit(conn, tenant_id)` — reads from account_tiers.limits |
| `get_plan_usage(conn, tenant_id)` — returned flat usage dict | `get_account_usage(conn, tenant_id)` — returns tier info + structured usage |
| N/A | `check_account_feature(conn, tenant_id, feature)` — new |
| N/A | `check_notification_channel_limit(conn, tenant_id)` — new |
| N/A | `check_user_limit(conn, tenant_id)` — new |
| N/A | `get_device_plan(conn, device_id)` — new |
| N/A | `check_device_limit(conn, device_id, resource, count)` — new |
| N/A | `check_device_feature(conn, device_id, feature)` — new |
| N/A | `check_sensor_limit(conn, tenant_id, device_id)` — new |
| N/A | `get_device_usage(conn, tenant_id, device_id)` — new |

## Verification

```bash
cd services/ui_iot && python3 -c "from middleware.entitlements import check_alert_rule_limit, check_account_feature, check_sensor_limit, get_account_usage, get_device_usage; print('OK')"
```
