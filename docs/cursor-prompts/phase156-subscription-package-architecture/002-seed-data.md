# Task 002 — Seed Data: Account Tiers, Device Plans, Subscriptions

## File

Create `db/migrations/108_seed_subscription_packages.sql`

## Purpose

Populate the new product definition tables with sample tiers, plans, and link them to the existing seed tenants and devices.
...
- Pricing is illustrative — Stripe price IDs are left NULL for seed data (not linked to a real Stripe account).
- The `free` account tier has $0 monthly — it's the entry point. Paid tiers are Growth ($49/mo), Scale ($149/mo), Enterprise ($499/mo).
- Device plans are per-device monthly: Basic ($2.99), Standard ($9.99), Premium ($24.99).
- A tenant's monthly bill = account tier fee + sum of all device plan fees. For the seed data: $49 (Growth) + $9.99 + $9.99 + $2.99 + $24.99 = $96.96/mo.

## Verification

```bash
psql "$DATABASE_URL" -c "SELECT tier_id, name, monthly_price_cents FROM account_tiers ORDER BY sort_order;"
# → 4 rows: free/growth/scale/enterprise

psql "$DATABASE_URL" -c "SELECT plan_id, name, monthly_price_cents FROM device_plans ORDER BY sort_order;"
# → 3 rows: basic/standard/premium

psql "$DATABASE_URL" -c "SELECT t.tenant_id, t.account_tier_id FROM tenants t WHERE t.tenant_id = 'acme-industrial';"
# → acme-industrial, growth

psql "$DATABASE_URL" -c "SELECT device_id, plan_id FROM device_registry WHERE tenant_id = 'acme-industrial' ORDER BY device_id;"
# → GW-001: standard, GW-002: standard, GW-003: basic, GW-004: premium

psql "$DATABASE_URL" -c "SELECT subscription_id, device_id, plan_id, status FROM device_subscriptions ORDER BY device_id;"
# → 4 rows, all ACTIVE
```
