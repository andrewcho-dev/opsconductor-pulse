# Task 001 — Schema Migration: Account Tiers + Device Plans

## File

Create `db/migrations/107_subscription_package_architecture.sql`

## Purpose

Replace the fragmented subscription/plan/tier model with two clean product definition tables: `account_tiers` (per tenant) and `device_plans` (per device). Drop all deprecated tables.
...
- The `device_subscriptions` table uses a partial unique index: only one non-terminal (not EXPIRED/CANCELLED) subscription per device. This allows subscription history.
- `device_registry.plan_id` is a denormalized convenience column. The authoritative plan is on `device_subscriptions`. Keep them in sync via application logic (same pattern as `active_device_count` was maintained before).
- `tenants.support_tier` and `tenants.sla_level` are NOT dropped yet — just deprecated. They can be removed in a future migration after all code references are updated.
- Carrier permissions are folded into this migration (was originally planned as standalone migration 107).

## Verification

```bash
DATABASE_URL="..." python3 db/migrate.py
# Should apply migration 107

psql "$DATABASE_URL" -c "\dt account_tiers; \dt device_plans; \dt device_subscriptions;"
# Should show all three tables

psql "$DATABASE_URL" -c "SELECT column_name FROM information_schema.columns WHERE table_name = 'tenants' AND column_name = 'account_tier_id';"
# Should return 1 row

psql "$DATABASE_URL" -c "SELECT column_name FROM information_schema.columns WHERE table_name = 'device_registry' AND column_name = 'plan_id';"
# Should return 1 row
```
