...
export async function listTemplates(params?: {
  ...
  const res = await apiGet<{ templates: DeviceTemplate[] }>(`${BASE}${qs ? `?${qs}` : ""}`);
...
export async function createTemplateMetric(
  templateId: number,
  data: TemplateMetricPayload,
): Promise<TemplateMetric> {
  return apiPost(`${BASE}/${templateId}/metrics`, data);
}

export async function updateTemplateMetric(
  templateId: number,
  metricId: number,
...
export async function createTemplateCommand(
  templateId: number,
  data: TemplateCommandPayload,
): Promise<TemplateCommand> {
