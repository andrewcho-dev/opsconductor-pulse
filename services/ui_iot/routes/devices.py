...
async def create_dynamic_group(body: DynamicGroupCreate, pool=Depends(get_db_pool)):
    ...
    if invalid_keys:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Unsupported filter keys: {', '.join(invalid_keys)}. "
    ...
    result = dict(row)
    result["query_filter"] = (
        json.loads(result["query_filter"])
        if isinstance(result["query_filter"], str)
        else result["query_filter"]
    )
    return result
...
@router.patch(
    "/device-groups/{group_id}/dynamic",
    dependencies=[require_permission("devices.groups.write")],
)
