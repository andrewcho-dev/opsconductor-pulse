...
async def operator_update_carrier_integration(
    ...
    if data.display_name is not None:
        add_set("display_name", data.display_name)
    if data.enabled is not None:
        add_set("enabled", data.enabled)
    if data.api_key is not None:
        add_set("api_key", data.api_key)
    if data.api_secret is not None:
        add_set("api_secret", data.api_secret)
    if data.api_base_url is not None:
        add_set("api_base_url", data.api_base_url)
    if data.account_id is not None:
        add_set("account_id", data.account_id)
    if data.sync_enabled is not None:
        add_set("sync_enabled", data.sync_enabled)
    if data.sync_interval_minutes is not None:
        add_set("sync_interval_minutes", data.sync_interval_minutes)
