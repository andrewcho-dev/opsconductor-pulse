-- Migration 112: Device sensors + device transports (instance-level model)
-- - device_sensors replaces legacy sensors table (with template/module links)
-- - device_transports replaces device_connections (protocol + connectivity split)
-- Also migrates existing data from legacy tables.
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_device_sensors_tenant_device ON device_sensors(tenant_id, device_id);
CREATE INDEX IF NOT EXISTS idx_device_sensors_template_metric ON device_sensors(template_metric_id);
CREATE INDEX IF NOT EXISTS idx_device_sensors_module ON device_sensors(device_module_id);
CREATE INDEX IF NOT EXISTS idx_device_sensors_status ON device_sensors(tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_device_sensors_source ON device_sensors(tenant_id, source);

-- RLS
ALTER TABLE device_sensors ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS device_sensors_tenant_isolation ON device_sensors;
CREATE POLICY device_sensors_tenant_isolation ON device_sensors
    USING (tenant_id = current_setting('app.tenant_id', true))
    WITH CHECK (tenant_id = current_setting('app.tenant_id', true));

DROP POLICY IF EXISTS device_sensors_operator_read ON device_sensors;
CREATE POLICY device_sensors_operator_read ON device_sensors
    FOR SELECT
    USING (current_setting('app.role', true) = 'operator');

DROP POLICY IF EXISTS device_sensors_service ON device_sensors;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON device_sensors TO pulse_operator;
GRANT USAGE, SELECT ON SEQUENCE device_sensors_id_seq TO pulse_app;
GRANT USAGE, SELECT ON SEQUENCE device_sensors_id_seq TO pulse_operator;

DROP TRIGGER IF EXISTS trg_device_sensors_updated_at ON device_sensors;
CREATE TRIGGER trg_device_sensors_updated_at
    BEFORE UPDATE ON device_sensors
    FOR EACH ROW
    EXECUTE FUNCTION update_sensors_timestamp();

-- Data migration from legacy sensors → device_sensors
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_device_transports_tenant_device ON device_transports(tenant_id, device_id);
CREATE INDEX IF NOT EXISTS idx_device_transports_protocol ON device_transports(tenant_id, ingestion_protocol);
CREATE INDEX IF NOT EXISTS idx_device_transports_carrier ON device_transports(carrier_integration_id)
    WHERE carrier_integration_id IS NOT NULL;

-- RLS
ALTER TABLE device_transports ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS device_transports_tenant_isolation ON device_transports;
CREATE POLICY device_transports_tenant_isolation ON device_transports
    USING (tenant_id = current_setting('app.tenant_id', true))
    WITH CHECK (tenant_id = current_setting('app.tenant_id', true));

DROP POLICY IF EXISTS device_transports_operator_read ON device_transports;
CREATE POLICY device_transports_operator_read ON device_transports
    FOR SELECT
    USING (current_setting('app.role', true) = 'operator');

DROP POLICY IF EXISTS device_transports_service ON device_transports;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON device_transports TO pulse_operator;
GRANT USAGE, SELECT ON SEQUENCE device_transports_id_seq TO pulse_app;
GRANT USAGE, SELECT ON SEQUENCE device_transports_id_seq TO pulse_operator;

DROP TRIGGER IF EXISTS trg_device_transports_updated_at ON device_transports;
CREATE TRIGGER trg_device_transports_updated_at
    BEFORE UPDATE ON device_transports
    FOR EACH ROW
    EXECUTE FUNCTION update_sensors_timestamp();

-- Data migration from legacy device_connections → device_transports
...
COMMENT ON COLUMN device_transports.connectivity_config IS 'Connectivity-specific config: Cellular {carrier, apn, iccid}; WiFi {ssid}';
